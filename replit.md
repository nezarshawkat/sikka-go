...
- **Connector geometry snapping** (`planner.ts` `adaptPlanToApi`, async): walk legs (`walking` profile) and taxi/tuktuk legs (`driving`) are snapped to the street network via `snapConnector()` (coord-cached, straight-line fallback if no Mapbox token). Transit legs keep their DB `route_path` polyline.
